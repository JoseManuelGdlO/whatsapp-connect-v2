...
| **WORKER_INBOUND_AUTO_READ** | Worker | Marca automáticamente como leídos los inbound (`readMessages`). Default: desactivado; `true` para activar. |
| **WORKER_OUTBOUND_MARK_READ_ON_SEND** | Worker | Acumula inbound en Redis y marca read al enviar outbound. Default: activado; `false` para desactivar. |
| **WORKER_INBOUND_AUTO_COMPOSING** | Worker | Composing al recibir inbound (legacy). Default: desactivado; `true` para activar. |
