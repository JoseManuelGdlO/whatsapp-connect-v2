...
| **"Esperando el mensaje" en WhatsApp** | Tiempo hasta respuesta: processingTimeMs en logs de inbound; cola `outbound_messages` cargada o Redis lento; por defecto no hay composing al recibir (solo al enviar); considerar `WORKER_INBOUND_ACK_MESSAGE` o `WORKER_INBOUND_AUTO_COMPOSING=true` (legacy). |
| **Tras deploy/reconexión, muchas respuestas o webhooks de golpe** | Log `[inbound-skip-stale]`; variable `WORKER_INBOUND_MAX_AGE_MS` (default 1 día); desvincular dispositivos de prueba con **Desvincular WhatsApp** (no solo pausar); revisar `WORKER_INBOUND_ACK_MESSAGE` y bot que responde a todo inbound. |
| **Decryption failed / Bad MAC / No matching sessions** | Stub + clearSenderAndReconnect en `inbound.ts`; en panel: **Reparar cifrado de chats** (`POST /devices/:id/reset-sender-sessions`); `authStateDb.ts`; que `WA_AUTH_ENC_KEY_B64` sea idéntica en todos los procesos que usan WaSession. |
